...
$(document).ready(function(){
	screenH = parseInt($('.container').height());
	screenW = parseInt($('.container').width()); 
	...
	$('#redirection .close').click(function () {
		$('#redirection').hide();
	});

	$('#raccourci img').dblclick(function(){
		if(jeuFini == false) {
		...
		$(this).attr('src', 'images/raccourci-message.png');
	});


	$('#raccourci img').click(function () {
		if(nbClick == 0){
			$(this).attr('src', 'images/raccourci-message-hover.png');
			nbClick++;
		}else {
			nbClick = 0;
			$(this).attr('src', 'images/raccourci-message.png');
...
function makePop(){
	...
	if(cpt==30){
		if($('.fenetre').length > 1){
			...
			if(jeuFini == "true") {
				$('#redirection').find('p').html("Vous avez obtenu un score de "+score+" points en 30 secondes, soit "+(score/10)+" fenêtres fermées.<br/><br/>Bravo, vous avez réussi à éliminer le virus. Vous pourez maintenant fermer cette fenêtre, et double cliquer sur le fichier message.txt pour l'ouvrir sans faire apparaître de virus, et le lire.<br/><br/>Si vous avez envie de battre votre score vous pouvez cliquer sur le bouton rejouer ci-dessous.");
			}else {
